...
peerIP         = '16.221.3.111'     #OMNIWHEELS SYSTEM'S  IP ADDRESS
...
def read_Pedalera():
    ...
    while(1):
        ...
        decoded_bytes1 = line1.decode('utf-8','ignore') #Decodifica los datos del serial
        data1 = (decoded_bytes1.replace('\r','')).replace('\n','') #Elimina los retorno de carro
        words = data1.split(",") # Agrega coma entre cada dato
        # Particion del mensaje
        if len(words) >= 4:
...
def connect_f():
    ...
    Bandera_Disconected = False
    s = serial.Serial(port='COM12',baudrate=115200, timeout=1) ## Pedalera
...
def validate_f():
    ...
    Pedalera_thread = threading.Thread(target= read_Pedalera)
